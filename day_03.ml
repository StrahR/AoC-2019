...
let naloga1 points1 =
  let rec aux best = function
    | [] -> best
    | ((a, b as x), _) :: xs ->
      if x <> (0, 0) && PairsMap.mem x points1
...
let naloga2 points1 =
  let rec aux best = function
    ...
    | ((a, b as x), d) :: xs ->
      if x <> (0, 0) && PairsMap.mem x points1
      then aux (min best ((PairsMap.find x points1) + d)) xs
      else aux best xs
  in aux 1000000
...
let _ =
  let build_list =
    let rec add_line acc k start length dir n = match dir with
      | 'R' -> if length >= 0 then add_line (PairsMap.add (start, k) n acc) k (start+1) (length-1) dir (n+1) else acc
      | 'L' -> if length >= 0 then add_line (PairsMap.add (start, k) n acc) k (start-1) (length-1) dir (n+1) else acc
      | 'D' -> if length >= 0 then add_line (PairsMap.add (k, start) n acc) k (start-1) (length-1) dir (n+1) else acc
      | 'U' -> if length >= 0 then add_line (PairsMap.add (k, start) n acc) k (start+1) (length-1) dir (n+1) else acc
      | _ -> PairsMap.empty
  ...
  let points1 = build_list (String.split_on_char ',' path1) in
  let points2 = build_list (String.split_on_char ',' path2) in
  let odgovor1 = naloga1 points1 (PairsMap.bindings points2)
  and odgovor2 = naloga2 points1 (PairsMap.bindings points2)
  in
